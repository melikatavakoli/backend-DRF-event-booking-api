...
class CreateTransactionAPIView(APIView):
    ...
    def post(self, request):
        ...
        if Transaction.objects.filter(booking=booking, status=TransactionStatus.pending).exists():
            return Response({
    ...
    def get(self, request):
        ...
        if response_json.get('status') == 'success':
            if response_json.get('amount', 0) == int(float(transaction.amount) * to_toman_rate):
                ...
                return redirect(f'{frontend_url}/payment-success?transaction_id={transaction.id}')
            else:
                return Response({'message': 'مبلغ تراکنش نامعتبر است!'}, status=400)
        else:
            transaction.mark_as_failed(reason=response_json.get('message', 'پرداخت ناموفق'))
            frontend_url = settings.FRONTEND_URL
            return redirect(f'{frontend_url}/payment-failed?transaction_id={transaction.id}')
...
class ApplyDiscountCodeAPIView(APIView):
    ...
    def post(self, request):
        ...
        try:
            discount = DiscountCode.objects.get(code=discount_code.upper(), is_active=True)
...
class TransactionAdminListView(ListAPIView):
    ...
    pagination_class = CustomLimitOffsetPagination
    filter_backends = [DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter]
    search_fields = ['transaction_no', 'description', 'user__first_name', 'user__last_name']
...
class TransactionUserListView(ListAPIView):
    ...
    def get_queryset(self):
        return Transaction.objects.filter(user=self.request.user).select_related('booking')
...
class AdminTransactionStatusUpdateAPIView(APIView):
    ...
    def patch(self, request, transaction_id):
        ...
        if new_status == TransactionStatus.success and transaction.booking and not transaction.booking.is_paid:

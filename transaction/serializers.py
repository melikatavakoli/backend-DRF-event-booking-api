...
class TransactionSerializer(serializers.ModelSerializer):
    ...
    def get_user(self, obj):
        ...
        return {
            ...
            'full_name': getattr(obj.user, 'full_name', f"{obj.user.first_name} {obj.user.last_name}"),
        }
    ...
    def get_booking_detail(self, obj):
        ...
        return {
            ...
            'concert_date': str(obj.booking.concert.date) if hasattr(obj.booking, 'concert') else None,
            'total_amount': str(obj.booking.total_amount),
            'tickets_count': obj.booking.tickets.count() if hasattr(obj.booking, 'tickets') else 0,
        }
...
class TransactionStatusUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate_status(self, value):
        ...
        valid_statuses = [TransactionStatus.success, TransactionStatus.failed, TransactionStatus.pending]
        if value not in valid_statuses:
            raise serializers.ValidationError(f"وضعیت باید یکی از این مقادیر باشد: {valid_statuses}")
        return value
...
class TransactionDetailSerializer(serializers.ModelSerializer):
    ...
    def get_booking_detail(self, obj):
        ...
        return {
            'id': obj.booking.id,
            'concert': obj.booking.concert.name if hasattr(obj.booking, 'concert') else None,
            'concert_date': str(obj.booking.concert.date) if hasattr(obj.booking, 'concert') else None,
            'total_amount': str(obj.booking.total_amount),

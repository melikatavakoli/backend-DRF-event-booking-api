...
class Transaction(GenericModel):
    ...
    stock_reduced = models.BooleanField(default=False,verbose_name="Stock Reduced Flag")
    ...
    def fee(self):
        ...
        if self.payment_method == 'online':
            return (self.calculated_amount * Decimal("0.005")).quantize(Decimal("0"), rounding=ROUND_HALF_UP)
        return Decimal("0")

    @property
    def total_amount_with_fee(self):
        """مجموع مبلغ تراکنش + کارمزد"""
        return (self.calculated_amount + self.fee).quantize(Decimal("0"), rounding=ROUND_HALF_UP)
...
class DiscountCode(GenericModel):
    ...
    def is_valid(self, amount=None):
        ...
        if amount and amount < self.min_purchase_amount:
            return False, f"حداقل مبلغ خرید برای این کد تخفیف {self.min_purchase_amount} تومان است"
